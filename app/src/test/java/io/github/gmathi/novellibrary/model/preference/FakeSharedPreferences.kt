 ...
 *
 * Implements the subset of the API exercised by [DataCenterDownloadStorageLocationTest],
 * [DataCenterReaderModeTest], and [DataCenterReaderDisplaySettingsTest]: `getString`/
 * `getBoolean`/`getInt` and an `Editor` supporting `putString`/`putBoolean`/`putInt`/`apply`.
 * All other members throw [NotImplementedError] since this fake is not intended for
 * general-purpose use.
 */
class FakeSharedPreferences : SharedPreferences {
    ...
    override fun getInt(key: String?, defValue: Int): Int = intValues[key] ?: defValue
    ...
    override fun contains(key: String?): Boolean = values.containsKey(key) || booleanValues.containsKey(key) || intValues.containsKey(key)
