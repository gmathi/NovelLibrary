...
import io.github.gmathi.novellibrary.databinding.FragmentBackupSettingsBinding
...
@AndroidEntryPoint
class BackupSettingsFragment : BaseViewBindingFragment<FragmentBackupSettingsBinding>(), GenericAdapter.Listener<String> {

    companion object {
        private const val TAG = "BackupSettingsFragment"
    ...
    override fun getLayoutId() = R.layout.fragment_backup_settings

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        setBinding(FragmentBackupSettingsBinding.bind(view))
        setRecyclerView()
                ...
                MaterialDialog(requireContext()).show {
                    title(text = item).listItemsSingleChoice(R.array.backup_frequency_options, initialSelection = selected) { _, which, _ ->
                        ...
                        if (dataCenter.backupFrequency != backupFrequency) {
                            val workRequest = if (backupFrequency != 0) periodicBackupWorkRequest(requireContext(), backupFrequency) else null
    ...
    override fun onDestroy() {
        if (workRequestId != null) WorkManager.getInstance(requireContext()).getWorkInfoByIdLiveData(workRequestId!!).removeObservers(this)
        super.onDestroy()
    }
        ...
        if (it.resultCode == Activity.RESULT_OK && it.data != null) {
            ...
            if (uri != null) {
                ...
                if (uri != oldUri) {
                    ...
                    try {
                        if (oldUri != null) requireContext().contentResolver.releasePersistableUriPermission(oldUri, readWriteFlags)
                    } catch (e: Exception) {
                        //Don't do anything
                    }

                    // Request for the new permissions to persist
                    it.data?.flags?.let { flags -> requireContext().contentResolver.takePersistableUriPermission(uri, flags and readWriteFlags) }
                        ?: requireContext().contentResolver.takePersistableUriPermission(uri, readWriteFlags)

                }

                val workRequest = oneTimeBackupWorkRequest(
                    requireContext(), uri, simpleText, database, preferences, files
                )
    ...
    private fun deleteFilesDialog() {
        MaterialDialog(requireContext()).show {
            ...
            positiveButton(R.string.clear) {
                val snackProgressBarManager = Utils.createSnackProgressBarManager(requireActivity().findViewById(android.R.id.content), requireActivity())
    ...
    private fun deleteFiles(dialog: SnackProgressBarManager) {
        launchIO {
            ...
            try {
                deleteDir(requireContext().cacheDir)
                deleteDir(requireContext().filesDir)
                dbHelper.removeAll()
