...
//import io.github.gmathi.novellibrary.service.download.DownloadNovelService.Companion.novelId
...
//
//        if (isDownloadPaused(novel.id)) return
...
//                if (dq != null && dq.status == Download.STATUS_IN_QUEUE) {
//
...
//    }
//
//    private fun getChapters(download: Download, @Suppress("UNUSED_PARAMETER") url: String): List<WebPage> = dbHelper.getAllWebPagesToDownload(download.novelName)
//    //{
...
//        dbHelper.getNovel(novelId)?.let { dbHelper.updateDownloadStatus(Download.STATUS_PAUSED, it.name!!) }
...
//    }
//
//    private fun isDownloadPaused(webPageId: Long): Boolean {
//        val status = dbHelper.getDownload(webPageId)?.status ?: return true
//        return status == Download.STATUS_PAUSED
//    }
//
//
//}