...
class AiTtsPreloader(private val context: Context, private val dataCenter: DataCenter) {
    ...
    fun preloadModel(modelId: String? = null) {
        ...
        // If already preloaded with the same model, skip
        if (preloadedPlayer != null && preloadedModelId == targetModelId) {
            Log.d(TAG, "Model $targetModelId already preloaded")
            return
        }
        ...
        scope.launch {
            try {
                ...
                // Step 4: Cache the initialized player
                // Destroy old player if exists
                preloadedPlayer?.destroy()
    ...
    fun getPreloadedPlayer(modelId: String? = null): AiAudioPlayer? {
        ...
        return if (preloadedModelId == targetModelId && preloadedPlayer != null) {
            Log.i(TAG, "Returning preloaded player for model: $targetModelId")
            val player = preloadedPlayer
            // Clear the cached player so it's not reused
            preloadedPlayer = null
            preloadedModelId = null
            player
        } else {
            Log.d(TAG, "No preloaded player available for model: $targetModelId")
            null
        }
    }
    ...
    fun isModelPreloaded(modelId: String? = null): Boolean {
        val targetModelId = modelId ?: dataCenter.ttsPreferences.aiModel
        return preloadedModelId == targetModelId && preloadedPlayer != null
    }
