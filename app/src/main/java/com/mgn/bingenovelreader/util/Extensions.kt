...
fun Fragment.enableLoadingView(enable: Boolean, otherView: View?) {
    if (!this.isRemoving && loadingLayout != null) {
        loadingLayout.visibility = if (enable) View.VISIBLE else View.INVISIBLE
        otherView?.visibility = if (enable) View.INVISIBLE else View.VISIBLE
    }
}

fun Fragment.setLoadingView(drawableRes: Int, message: String?) {
    Glide.with(this).load(drawableRes).into(loadingImageView)
    loadingTextView.typeface = Typeface.createFromAsset(activity.assets, "font/roboto_regular" + ".ttf")
    loadingTextView.text = message
}
